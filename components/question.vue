			...
			<view class="mainBox radius  bg-white">
				<view v-if="hasData">
					<view class="u-flex u-row-between">
						...
						<u--text type="info" align="right" :text=" questioncp[num].choice_type==0?'单选题':'多选题'">
						</u--text>
					...
					<view class="animation-reverse" :class="animation" data-class="slide-rights">
						<view class="mentalTitle">
							<mp-html :content="questioncp[num].choiceqst_content" />
						</view>
						<view class="mentalList" v-for="(item, index) in questioncp[num].answer"
					...
					<u-tabbar :fixed="true" :placeholder="true" :safeAreaInsetBottom="true">
						<view class="btns u-flex"
							:class="{'u-row-right':num!=questioncp.length,'u-row-between':num!=0}">
							<view class="btn" @click="toPre" :class='{disabled:num==0}'>
								上一题
							</view>
							<view class="btn" v-if="(num+1)!=questioncp.length" @click="toNext"
								:class='{disabled:disabledBtn}'>
								下一题
								<!-- <u-button :disabled="disabledBtn" text="下一题" @click="toNext"></u-button> -->
							</view>
							<view class="btn" v-if="(num+1)==questioncp.length" :class='{disabled:disabledBtn}'
								@click="toEnd">
								提交
	...
	export default {
		...
		methods: {
			Toggle(info, index) {
				...
				let option = this.questioncp[this.num].answer //每一题题目的选项
				...
				option.forEach((item, index) => {
					if (item.id == info.id) { //如果选中的与数据里面的相等就改变他的样式
						...
						this.$set(this.questioncp[this.num].answer[index], 'checked', !checked)


					} else if (questioncp.choice_type == 0) { //如果不是多选 只能选中一个 其他的都为false
						this.$set(this.questioncp[this.num].answer[index], 'checked', false)
					...
					// 判断正确与否
					if (!((item.right_answer && item.checked) || (!item.right_answer && !item.checked))) {
						right = false;
					}
				});
				this.hideNotify()
				this.$set(this.answer, this.questioncp[this.num].id, right)
			...
			toNext() {
				...
				if (!this.answer.hasOwnProperty(this.questioncp[this.num].id)) {
					this.showNotify();
					return;

				} else {
					if (!this.answer[this.questioncp[this.num].id]) {
						this.showNotify({
							type: 'error',
							message: '选项不正确,请重新选择'
						});
