			...
			<view class="mainBox radius  bg-white">
				<view v-if="hasData">
					<view class="u-flex u-row-between">
						<view class="u-flex">
							...
							<view class="">
								<u--text type="info" :text="questioncp[0].list.length"></u--text>
							</view>
						</view>
						<u--text type="info" align="right" :text=" questioncp[0].list[num].tm.choiceType==0?'单选题':'多选题'">
						</u--text>
					...
					<view class="animation-reverse" :class="animation" data-class="slide-rights">
						<view class="mentalTitle">
							<mp-html :content="questioncp[0].list[num].tm.choiceqstContent" />
						</view>
						<view class="mentalList" v-for="(item, index) in questioncp[0].list[num].answer"
					...
					<u-tabbar :fixed="true" :placeholder="true" :safeAreaInsetBottom="true">
						<view class="btns u-flex"
							:class="{'u-row-right':num!=questioncp[0].list.length,'u-row-between':num!=0}">
							<view class="btn" @click="toPre" :class='{disabled:num==0}'>
								上一题
							</view>
							<view class="btn" v-if="(num+1)!=questioncp[0].list.length" @click="toNext"
								:class='{disabled:disabledBtn}'>
								下一题
								<!-- <u-button :disabled="disabledBtn" text="下一题" @click="toNext"></u-button> -->
							</view>
							<view class="btn" v-if="(num+1)==questioncp[0].list.length" :class='{disabled:disabledBtn}'
								@click="toEnd">
								提交
				...
				<view v-if="noData" class="success">
					<view class="image-box">
						<u--image :showLoading="true" src="http://cdn.cestech.com.cn/BkpKnowledge/img/a908858bf4e44fa1ace68675c8e74a47.png" width="140px"
							height="140px">
						</u--image>
					</view>
...
<script>
	...
	export default {
		...
		watch: {
			question: {
				handler(val) {
					let that=this
					that.questioncp = val
					// console.log(that.questioncp.list[0].tm.choiceType,'答题数据')
				},
				deep: true,
				immediate: true
			},
		...
		computed: {
			disabledBtn() {
				if (this.questioncp[0].list.length <= 0) {
					return true;
				}
		...
		methods: {
			Toggle(info, index) {
				let questioncp = this.questioncp[0].list //每题
				let option = this.questioncp[0].list[this.num].answer //每一题题目的选项
				...
				option.forEach((item, index) => {
					if (item.id == info.id) { //如果选中的与数据里面的相等就改变他的样式
						...
						this.$set(this.questioncp[0].list[this.num].answer[index], 'checked', !checked)


					} else if (questioncp[this.num].tm.choiceType == 0) { //如果不是多选 只能选中一个 其他的都为false
						this.$set(this.questioncp[0].list[this.num].answer[index], 'checked', false)
				...
				this.hideNotify()
				this.$set(this.answer, this.questioncp[0].list[this.num].tm.id, right)
			...
			toNext() {
				...
				if (!this.answer.hasOwnProperty(this.questioncp[0].list[this.num].tm.id)) {
					this.showNotify();
					return;

				} else {
					if (!this.answer[this.questioncp[0].list[this.num].tm.id]) {
						this.showNotify({
							type: 'error',
							message: '选项不正确,请重新选择'
						});
			...
			toEnd() {
				...
				// if (this.details[0].points_acquisition_rule === 3) {
				// 	add_points({
				// 		source_type: 3,
				// 		get_type: 2,
				// 		source_id: this.details[0].id,
				// 		points: this.details[0].points,
				// 		title: this.details[0].knowledge_title
				// 	}).then(res => {
				// 		uni.showToast({
				// 			icon: 'success',
				// 			title: '恭喜获得答题积分'
